"""重建 app.js：保留工具函数和辅助函数，替换 6 个旧路由为 7 个新路由"""
import re

src_path = r"d:\software\investment-os\static\app.js"
with open(src_path, "r", encoding="utf-8") as f:
    lines = f.readlines()

# 找到切分点
# Part 1: 1 ~ 172 行（第 173 行是总览路由开始）
# Part 3: 889 行 ~ 结尾（财报详情与推送辅助函数、onload）
part1 = lines[:172]
part3 = lines[888:]  # index 888 是第 889 行（因为 0-based）

print(f"Part 1: {len(part1)} lines, Part 3: {len(part3)} lines")
...
print(f"✅ app.js 重建完成: 总行数 = {len(part1) + new_routes.count(chr(10)) + len(part3)}")
print(f"  Part 1: {len(part1)} 行")
print(f"  Part 2 (新路由): {new_routes.count(chr(10))} 行")
print(f"  Part 3 (辅助函数): {len(part3)} 行")
